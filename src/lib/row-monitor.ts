...
function initialize(symbol: string) {
	...
	if (!root || !(root instanceof HTMLTableSectionElement)) {
		console.error(
			`No tbody element matching selector "${selector}" found: ${root}`
		);
		return;
	}

	const makeSnapShot = () => new Set(root.querySelectorAll('tr').values());
...
function scheduleCompare(symbol: string) {
	if (isServer) return;

	if (!bag) {
		bag = initialize(symbol);
